"""Train two-layer Sim-DETR + LS-DQ-CGP + Existence on Soccer-GMR."""
...
def weighted_model_loss(losses, criterion):
    terms = [
...
def alignment_losses(outputs, model_inputs, targets, opt):
    ...
    if positive.any():
        ...
        chunks = [
            VTCLoss()(
                text[start:start + opt.vtc_group_size],
                video[start:start + opt.vtc_group_size],
            )
...
def train_epoch(model, criterion, loader, optimizer, opt, epoch):
    ...
    for batch_index, (_, batched) in enumerate(tqdm(loader, desc=f"epoch {epoch + 1}")):
        ...
        model_loss = weighted_model_loss(loss_dict, criterion)
        align_loss, ctc, vtc = alignment_losses(outputs, model_inputs, targets, opt)
        loss = model_loss + align_loss
        if not torch.isfinite(loss):
            raise FloatingPointError(
                f"Non-finite loss at epoch={epoch + 1}, batch={batch_index}"
            )
...
def main(argv=None):
    ...
    model, criterion = build_soccer_gmr_ls_dq_cgp(opt)
    model.to(opt.device)
