"""State-dict-preserving DQ-CGP hook for Sim-DETR's native decoder.
...
Only one operation is added: after the configured native decoder layer has
updated the reference points and emitted its pre-adapter auxiliary state, the
DQ-CGP adapter refines the content state passed to the following layer.
"""
...
def _set_adapter(self, adapter) -> None:
    """Update the non-registered adapter reference used by the hook."""
    object.__setattr__(self, "_dq_cgp_adapter", adapter)
...
def _dq_cgp_decoder_forward(
    ...
    """Native Sim-DETR decoder loop with one inter-layer DQ-CGP call."""
    output = tgt
    nq, bs, _ = output.size()
    ...
    for layer_id, layer in enumerate(self.layers):
        ...
        if layer_id == self._dq_cgp_after_layer:
            adapter = self._dq_cgp_adapter
            if adapter is None:
...
def install_dq_cgp_decoder_hook(decoder, adapter, after_layer: int = 0):
    """Install DQ-CGP on an existing decoder without replacing that module.

    The adapter is deliberately stored as a plain Python reference: it is
    registered exactly once by ``SimDETRWithDQCGP`` and therefore appears only
    under ``query_cgp.*`` in the model state dict.
    """
    after_layer = _validate_after_layer(decoder, after_layer)
    ...
    object.__setattr__(decoder, "_dq_cgp_adapter", adapter)
    object.__setattr__(decoder, "_dq_cgp_adapter_kwargs", {})
    object.__setattr__(decoder, "_dq_cgp_after_layer", after_layer)
