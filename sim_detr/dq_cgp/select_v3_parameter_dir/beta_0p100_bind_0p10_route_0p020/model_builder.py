"""Model builder for Sim-DETR + DQ-CGP.

This module mirrors ``sim_detr.model.build_model`` but returns a
``SimDETRWithDQCGP`` instead of ``SimDETR``.  It also extends the criterion's
...
def build_model_dq_cgp(args) -> Tuple[SimDETRWithDQCGP, DQCGPSetCriterion]:
    """Build Sim-DETR + DQ-CGP model and criterion.

    Mirrors ``sim_detr.model.build_model`` but produces ``SimDETRWithDQCGP``
    ...
    if args.a_feat_dir is None:
        model = SimDETRWithDQCGP(**common_kwargs)
    else:
        model = SimDETRWithDQCGP(
