"""SimDETRWithDQCGP: Sim-DETR + DQ-CGP inter-layer adapter.

This class **subclasses SimDETR** without touching any original source file.
It injects DQ-CGP between decoder layer 1 and decoder layer 2 by:
...
3. Overriding ``forward`` to:
   ...
   - Expose DQ-CGP diagnostics and losses in the output dict.
...
class SimDETRWithDQCGP(SimDETR):
    """Sim-DETR extended with a DQ-CGP inter-decoder-layer adapter.
    ...
    query_cgp_after_layer : int
        DQ-CGP is inserted after this decoder layer (0-indexed).  Default 0.
    ...
    def __init__(
        ...
        if use_query_cgp:
            ...
            # ---- 1. Instantiate DQ-CGP (owned here, registered as submodule) ----
            ...
            install_dq_cgp_decoder_hook(
                ...
                adapter=self.query_cgp,
                after_layer=query_cgp_after_layer,
            )
    ...
    def forward(
        ...
        if (
            self.use_query_cgp
            and self.query_cgp is not None
            and self.query_cgp.last_output is not None
        ):
            qout = self.query_cgp.last_output
            out["query_cgp_temporal_attention"] = qout.temporal_attention  # [B,Q,T]
            out["query_cgp_basis_weights"] = qout.basis_weights            # [B,Q,K]
            out["query_cgp_video_mask"] = src_vid_mask.bool()              # [B,T]
    ...
    def load_from_baseline_checkpoint(
        cls,
        checkpoint_path: str,
        model: "SimDETRWithDQCGP",
        device: str = "cpu",
    ) -> None:
        """Initialise DQ-CGP model from a baseline Sim-DETR checkpoint.

        Only ``query_cgp.*`` keys are allowed to be missing.  All other
        original Sim-DETR weights must be present and unmodified.
        ...
        source_dq_keys = [
            key for key in state_dict if key.startswith("query_cgp.")
        ]
        if source_dq_keys:
            raise RuntimeError(
        ...
        disallowed_missing = [k for k in missing if not k.startswith("query_cgp.")]
        ...
        n_cgp = len([k for k in missing if k.startswith("query_cgp.")])
        print(
            f"[DQ-CGP] Loaded baseline checkpoint. "
            f"{n_cgp} query_cgp.* tensors keep their DQ-CGP initial values."
        )


__all__ = ["SimDETRWithDQCGP"]
