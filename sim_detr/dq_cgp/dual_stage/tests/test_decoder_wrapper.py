...
from sim_detr.dq_cgp.dual_stage.decoder_wrapper import (
    install_dq_cgp_decoder_hook,
    install_dq_cgp_decoder_hooks,
    remove_dq_cgp_decoder_hook,
    set_dq_cgp_adapter_kwargs,
)
from sim_detr.dq_cgp.dual_stage.query_cgp import DETRQueryCGP
...
class TestNativeDecoderHook:
    ...
    def test_dual_adapters_run_once_at_layers_zero_and_two(self):
        class CountingIdentityAdapter(nn.Module):
            def __init__(self):
                super().__init__()
                self.calls = 0
        ...
        decoder = make_decoder()
        early = CountingIdentityAdapter()
        late = CountingIdentityAdapter()
        inputs = make_inputs()
        install_dq_cgp_decoder_hooks(decoder, {0: early, 2: late})
        set_dq_cgp_adapter_kwargs(decoder, query_semantic=inputs[-1])
        run_decoder(decoder, inputs)
        assert early.calls == 1
        assert late.calls == 1

    def test_late_adapter_changes_only_final_decoder_state(self):
        early_only = make_decoder()
        dual = copy.deepcopy(early_only)
        inputs = make_inputs()
        early = make_cgp(beta=0.05)
        install_dq_cgp_decoder_hook(early_only, early, after_layer=0)
        install_dq_cgp_decoder_hooks(
            dual,
            {0: copy.deepcopy(early), 2: make_cgp(beta=0.05)},
        )
        set_dq_cgp_adapter_kwargs(early_only, query_semantic=inputs[-1])
        set_dq_cgp_adapter_kwargs(dual, query_semantic=inputs[-1])

        early_hs, early_refs = run_decoder(early_only, inputs)
        dual_hs, dual_refs = run_decoder(dual, inputs)

        assert torch.equal(dual_hs[:3], early_hs[:3])
        assert not torch.equal(dual_hs[3], early_hs[3])
        assert torch.equal(dual_refs, early_refs)

    def test_dual_beta_zero_is_exact_native_decoder_identity(self):
        baseline = make_decoder()
        dual = copy.deepcopy(baseline)
        inputs = make_inputs()
        install_dq_cgp_decoder_hooks(
            dual,
            {0: make_cgp(beta=0.0), 2: make_cgp(beta=0.0)},
        )
        set_dq_cgp_adapter_kwargs(dual, query_semantic=inputs[-1])
        baseline_hs, baseline_refs = run_decoder(baseline, inputs)
        dual_hs, dual_refs = run_decoder(dual, inputs)
        assert torch.equal(dual_hs, baseline_hs)
        assert torch.equal(dual_refs, baseline_refs)
...
class TestCheckpointCompatibility:
    def test_same_seed_preserves_all_single_stage_initial_tensors(self):
        ...
        assert any(key.startswith("query_cgp_late.") for key in dual_state)
    ...
    def test_baseline_keys_load_with_only_query_cgp_missing(self):
        ...
        assert missing
        assert all(
            key.startswith(("query_cgp.", "query_cgp_late."))
            for key in missing
        )
        assert any(key.startswith("query_cgp.") for key in missing)
        assert any(key.startswith("query_cgp_late.") for key in missing)
        assert not any("decoder.decoder" in key for key in dq_model.state_dict())
        assert set(dq_model.query_cgp.parameters()).isdisjoint(
            set(dq_model.query_cgp_late.parameters())
        )

    def test_dq_checkpoint_strict_round_trip(self):
        source = SimDETRWithDualDQCGP(
            transformer=make_transformer(),
            use_query_cgp=True,
            **common_model_kwargs(),
        )
        target = SimDETRWithDualDQCGP(
            transformer=make_transformer(),
            use_query_cgp=True,
            **common_model_kwargs(),
        )
    ...
    def test_baseline_checkpoint_helper(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            SimDETRWithDualDQCGP.load_from_baseline_checkpoint(
