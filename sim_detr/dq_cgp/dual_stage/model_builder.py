"""Model builder for Sim-DETR + dual-stage DQ-CGP.

This module mirrors ``sim_detr.model.build_model`` but returns a
``SimDETRWithDualDQCGP`` instead of ``SimDETR``.  It also extends the criterion's
``loss_map`` and ``weight_dict`` with the DQ-CGP losses.

Usage in a training script::

    from sim_detr.dq_cgp.dual_stage.model_builder import build_model_dq_cgp
...
from .sim_detr_dq_cgp import SimDETRWithDualDQCGP
...
def add_dq_cgp_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    g.add_argument(
        ...
        default=2,
        help=(
            "Insert the independent late DQ-CGP after this decoder layer "
    ...
    g.add_argument(
        "--eval_query_cgp_early_beta",
        type=float,
        default=None,
        help="Inference-only beta override applied after strict checkpoint load.",
    )
    g.add_argument(
        "--eval_query_cgp_late_beta",
        type=float,
        default=None,
        help="Inference-only late beta override after strict checkpoint load.",
    )
    return parser
...
def build_model_dq_cgp(args) -> Tuple[SimDETRWithDualDQCGP, DQCGPSetCriterion]:
    """Build Sim-DETR + DQ-CGP model and criterion.

    Mirrors ``sim_detr.model.build_model`` but produces ``SimDETRWithDualDQCGP``
    ...
    if args.a_feat_dir is None:
        model = SimDETRWithDualDQCGP(**common_kwargs)
    else:
        model = SimDETRWithDualDQCGP(
