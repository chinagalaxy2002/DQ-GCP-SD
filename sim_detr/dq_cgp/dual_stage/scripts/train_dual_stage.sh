...
t_feat_type=clip
results_root=results_dq_cgp/dual_stage_flexible
exp_id=sim_detr_dual_dq_cgp
...
# ---------------------------------------------------------------------------
# Dual-stage DQ-CGP hyperparameters
# ---------------------------------------------------------------------------
...
query_cgp_beta=0.05
query_cgp_after_layer=0     # D1 → DQ → D2 → D3 → D4
query_cgp_late_after_layer=2 # D1 → DQ → D2 → D3 → DQ → D4
...
# Run
# ---------------------------------------------------------------------------
"${python_bin}" -m sim_detr.dq_cgp.dual_stage.train_dq_cgp \
  --seed ${seed} \
