#!/usr/bin/env bash
# Controlled stacking experiment:
#   single-stage V3  vs.  dual-stage DQ-CGP (after D1 and D3)
#
# This script deliberately has no --init_from/--resume path.  It matches the
# single-stage scratch protocol and changes only the second adapter.

set -euo pipefail

if (( $# != 0 )); then
  echo "This canonical scratch script accepts no CLI arguments." >&2
  echo "Use DUAL_DQCGP_* variables documented in dual_stage/README.md." >&2
  exit 2
fi
...
seed="${DUAL_DQCGP_SEED:-2017}"
gpu_id="${DUAL_DQCGP_GPU_ID:-0}"
num_workers="${DUAL_DQCGP_NUM_WORKERS:-4}"
results_root="${DUAL_DQCGP_RESULTS_ROOT:-${repo_root}/results_dq_cgp/dual_stage_scratch/runs}"
exp_id="${DUAL_DQCGP_EXP_ID:-sim_detr_dual_dq_cgp_d1_d3_scratch_seed${seed}}"
...
[[ "${exp_id}" =~ ^[A-Za-z0-9._-]+$ ]] || {
  echo "DUAL_DQCGP_EXP_ID may contain only letters, numbers, '.', '_' and '-'." >&2
  exit 2
}
...
echo "Starting Sim-DETR + dual-stage DQ-CGP from scratch"
...
# method change is the independent late adapter after D3; early/late auxiliary
# losses are averaged so the global 0.2/0.01 loss budget stays unchanged.
exec "${python_bin}" -m sim_detr.dq_cgp.dual_stage.train_dq_cgp \
  --dset_name hl \
  --ctx_mode video_tef \
