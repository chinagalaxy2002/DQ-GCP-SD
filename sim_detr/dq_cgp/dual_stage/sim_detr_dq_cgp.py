"""Sim-DETR with two independent inter-layer DQ-CGP adapters.
...
3. Overriding ``forward`` to:
   ...
   - Expose separate early/late diagnostics for averaged V3 losses.
...
class SimDETRWithDualDQCGP(SimDETR):
    """Sim-DETR extended with independent early and late DQ-CGP adapters.
    ...
    def __init__(
        ...
        if use_query_cgp:
            ...
            # Independent parameters isolate candidate adaptation at each depth.
            ...
            self.query_cgp_late = DETRQueryCGP(
                hidden_dim=hidden_dim,
                num_basis=query_cgp_num_basis,
                prompt_length=query_cgp_prompt_length,
                router_hidden_dim=query_cgp_router_hidden_dim,
            ...
            install_dq_cgp_decoder_hooks(
                ...
                adapters_by_layer={
                    ...
                    self.query_cgp_late_after_layer: self.query_cgp_late,
                },
            )
    ...
    def forward(
        ...
        if self.use_query_cgp and self.query_cgp is not None:
            ...
            self.query_cgp_late.clear_diagnostics()
        ...
        # ------------------------------------------------------------------
        if self.use_query_cgp and self.query_cgp is not None:
            early_output = self.query_cgp.last_output
            late_output = self.query_cgp_late.last_output
    ...
    def load_from_baseline_checkpoint(
        ...
        Only ``query_cgp.*`` and ``query_cgp_late.*`` keys may be missing.
        All original Sim-DETR weights must be present and unmodified.
        ...
        dq_prefixes = ("query_cgp.", "query_cgp_late.")
        source_dq_keys = [key for key in state_dict if key.startswith(dq_prefixes)]
        if source_dq_keys:
            raise RuntimeError(
        ...
        disallowed_missing = [k for k in missing if not k.startswith(dq_prefixes)]
        ...
        n_cgp = len([k for k in missing if k.startswith(dq_prefixes)])
        print(
            f"[DQ-CGP] Loaded baseline checkpoint. "
            f"{n_cgp} early/late DQ-CGP tensors keep their initial values."
        )


__all__ = ["SimDETRWithDualDQCGP"]
