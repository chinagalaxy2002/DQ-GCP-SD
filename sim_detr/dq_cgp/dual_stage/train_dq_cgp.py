...
def _load_baseline_weights(model, checkpoint_path):
    ...
    and only the two newly introduced DQ-CGP prefixes may be absent.
    """
    ...
    if any(key.startswith(dq_prefixes) for key in normalized_source_keys):
        raise RuntimeError(
            "--init_from expects a baseline Sim-DETR checkpoint without "
            "DQ-CGP tensors. Use --resume for a dual-stage checkpoint."
        )
...
def setup_model_dq_cgp(opt):
    ...
    if getattr(opt, "use_query_cgp", False) and model.query_cgp is not None:
        early_params, _ = count_parameters(model.query_cgp, verbose=False)
        late_params, _ = count_parameters(model.query_cgp_late, verbose=False)
        cgp_params = early_params + late_params
        logger.info(
