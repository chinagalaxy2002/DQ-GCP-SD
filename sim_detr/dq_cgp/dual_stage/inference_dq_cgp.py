"""Strict checkpoint inference for Sim-DETR + dual-stage DQ-CGP.
...
def _requested_eval_overrides(argv):
    ...
    parser.add_argument("--eval_query_cgp_early_beta", type=float, default=None)
    parser.add_argument("--eval_query_cgp_late_beta", type=float, default=None)
...
def parse_inference_options(argv=None):
    ...
    opt.eval_query_cgp_early_beta = overrides.eval_query_cgp_early_beta
    opt.eval_query_cgp_late_beta = overrides.eval_query_cgp_late_beta
...
def setup_model_dq_cgp_inference(opt):
    ...
    if opt.eval_query_cgp_early_beta is not None:
        model.query_cgp.set_beta(opt.eval_query_cgp_early_beta)
    if opt.eval_query_cgp_late_beta is not None:
        model.query_cgp_late.set_beta(opt.eval_query_cgp_late_beta)
    model.to(opt.device)
