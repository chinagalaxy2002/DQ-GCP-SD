...
Run from the Sim-DETR project root:
    PYTHONPATH=. python -m pytest sim_detr/dq_cgp/tied_dual_stage/tests/test_loss.py -v
"""

import torch
from torch import nn

from sim_detr.dq_cgp.tied_dual_stage.loss import (
    compute_binding_loss,
    compute_routing_loss,
    loss_query_cgp,
)
from sim_detr.dq_cgp.tied_dual_stage.model_builder import DQCGPSetCriterion
...
class TestCombinedLoss:
    ...
    def test_stage_losses_are_separate_then_exactly_averaged(self):
        ...
        assert torch.allclose(
            result["loss_query_cgp_bind"], 0.2 * 0.5 * late_bind, atol=1e-6
        )
        assert torch.allclose(
            result["loss_query_cgp_route"], 0.01 * 0.5 * late_route, atol=1e-6
        )
