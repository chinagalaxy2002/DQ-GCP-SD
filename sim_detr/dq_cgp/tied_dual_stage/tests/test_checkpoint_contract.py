"""Checkpoint identity tests for the tied dual-stage experiment."""
...
from sim_detr.dq_cgp.tied_dual_stage import inference_dq_cgp
from sim_detr.dq_cgp.tied_dual_stage.sim_detr_dq_cgp import ARCHITECTURE_ID
from sim_detr.dq_cgp.tied_dual_stage.checkpoint_contract import (
    CHECKPOINT_CONFIG_KEYS,
    validate_tied_checkpoint,
)
from sim_detr.dq_cgp.tied_dual_stage.train_dq_cgp import _checkpoint_payload


EXPECTED_ARCHITECTURE_ID = "sim_detr_tied_dual_dq_cgp_v1"
...
class TestCheckpointArchitectureContract:
    ...
    def test_contract_accepts_exact_config_and_rejects_semantic_mismatch(self):
        ...
        current = _contract_config(query_cgp_late_after_layer=1)
        with pytest.raises(RuntimeError, match="late_after_layer"):
