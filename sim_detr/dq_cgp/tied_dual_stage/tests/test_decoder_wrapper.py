...
from sim_detr.dq_cgp.tied_dual_stage.query_cgp import DETRQueryCGP
...
class TestNativeDecoderHook:
    ...
    def test_same_adapter_runs_once_at_each_tied_insertion_layer(self):
        ...
        inputs = make_inputs()
        install_dq_cgp_decoder_hooks(decoder, {0: shared, 2: shared})
        set_dq_cgp_adapter_kwargs(decoder, query_semantic=inputs[-1])
        run_decoder(decoder, inputs)
        adapters = decoder._dq_cgp_adapters_by_layer
        assert adapters[0] is shared
        assert adapters[2] is shared
        assert adapters[0] is adapters[2]
        assert shared.calls == 2

    def test_tied_calls_snapshot_early_and_late_diagnostics(self):
        decoder = make_decoder()
        ...
        assert not torch.equal(
            early.temporal_attention, late.temporal_attention
        )
    ...
    def test_late_adapter_changes_only_final_decoder_state(self):
        ...
        assert torch.equal(tied_hs[:3], early_hs[:3])
        assert not torch.equal(tied_hs[3], early_hs[3])
        assert torch.equal(tied_refs, early_refs)
    ...
    def test_tied_beta_zero_is_exact_identity_and_clears_snapshots(self):
        ...
        inputs = make_inputs()
        shared = make_cgp(beta=0.05)
        install_dq_cgp_decoder_hooks(tied, {0: shared, 2: shared})
        set_dq_cgp_adapter_kwargs(tied, query_semantic=inputs[-1])

        # Populate both slots first.  The beta=0 pass must not leak these stale
        # diagnostic graphs into the next loss computation.
        run_decoder(tied, inputs)
        assert set(get_dq_cgp_stage_outputs(tied)) == {0, 2}
        shared.set_beta(0.0)
    ...
    def test_both_tied_call_outputs_and_all_shared_params_receive_gradients(self):
        ...
        assert stage_outputs[2].adapted_state is call_outputs[1]
...
class TestCheckpointCompatibility:
    ...
    def test_baseline_keys_load_with_only_query_cgp_missing(self):
        ...
        dq_model = SimDETRWithTiedDualDQCGP(
            transformer=make_transformer(),
            use_query_cgp=True,
            **common_model_kwargs(),
        )
    ...
    def test_single_and_tied_checkpoints_load_strictly_both_directions(self):
        ...
        tied_target = SimDETRWithTiedDualDQCGP(
            transformer=make_transformer(),
            use_query_cgp=True,
            **common_model_kwargs(),
        )
        ...
        tied_source = SimDETRWithTiedDualDQCGP(
            transformer=make_transformer(),
            use_query_cgp=True,
            **common_model_kwargs(),
        )
    ...
    def test_baseline_checkpoint_helper(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            SimDETRWithTiedDualDQCGP.load_from_baseline_checkpoint(
    ...
    def test_init_from_rejects_dq_checkpoint(self):
        source = SimDETRWithTiedDualDQCGP(
            transformer=make_transformer(),
            use_query_cgp=True,
            **common_model_kwargs(),
        )
        target = SimDETRWithTiedDualDQCGP(
            transformer=make_transformer(),
            use_query_cgp=True,
            **common_model_kwargs(),
        )
