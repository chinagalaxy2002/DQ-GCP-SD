"""State-dict-preserving tied dual-stage DQ-CGP hook for Sim-DETR.
...
The same registered DQ-CGP module refines content after decoder layers 1 and 3
(0-indexed layers 0 and 2).  The hook snapshots diagnostics immediately after
each call so the late call cannot overwrite the early call's training signal.
"""
...
def _dq_cgp_decoder_forward(
    ...
    intermediate = []
    object.__setattr__(self, "_dq_cgp_stage_outputs", {})
    ...
    for layer_id, layer in enumerate(self.layers):
        ...
        if layer_id in self._dq_cgp_disabled_layers:
            adapter = None
        if adapter is not None:
            ...
            stage_output = getattr(adapter, "last_output", None)
            if stage_output is not None:
                self._dq_cgp_stage_outputs[layer_id] = stage_output
        else:
...
def install_dq_cgp_decoder_hooks(decoder, adapters_by_layer):
    ...
    object.__setattr__(decoder, "_dq_cgp_stage_outputs", {})
    object.__setattr__(decoder, "_dq_cgp_disabled_layers", frozenset())
...
def remove_dq_cgp_decoder_hook(decoder):
    ...
    for name in (
        ...
        "_dq_cgp_stage_outputs",
        "_dq_cgp_disabled_layers",
        _HOOK_FLAG,
        "set_adapter",
        "set_adapters",
        "set_adapter_kwargs",
...
def set_dq_cgp_disabled_layers(decoder, layers=()) -> None:
    """Disable selected calls for inference-only tied-stage ablations."""
    if not getattr(decoder, _HOOK_FLAG, False):
        raise RuntimeError("DQ-CGP decoder hook is not installed")
    disabled = frozenset(int(layer) for layer in layers)
    unknown = disabled.difference(decoder._dq_cgp_adapters_by_layer)
    if unknown:
        raise ValueError(f"cannot disable unconfigured adapter layers: {sorted(unknown)}")
    object.__setattr__(decoder, "_dq_cgp_disabled_layers", disabled)


def get_dq_cgp_stage_outputs(decoder):
    """Return per-layer diagnostic snapshots from the latest forward."""
    if not getattr(decoder, _HOOK_FLAG, False):
        raise RuntimeError("DQ-CGP decoder hook is not installed")
    return dict(decoder._dq_cgp_stage_outputs)
