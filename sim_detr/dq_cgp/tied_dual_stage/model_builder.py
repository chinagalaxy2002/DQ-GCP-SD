"""Model builder for Sim-DETR + tied dual-stage DQ-CGP.

This module mirrors ``sim_detr.model.build_model`` but returns a
``SimDETRWithTiedDualDQCGP`` instead of ``SimDETR``.  It also extends the criterion's
``loss_map`` and ``weight_dict`` with the DQ-CGP losses.

Usage in a training script::

    from sim_detr.dq_cgp.tied_dual_stage.model_builder import build_model_dq_cgp
...
from .sim_detr_dq_cgp import SimDETRWithTiedDualDQCGP
...
def build_model_dq_cgp(args) -> Tuple[SimDETRWithTiedDualDQCGP, DQCGPSetCriterion]:
    """Build Sim-DETR + DQ-CGP model and criterion.

    Mirrors ``sim_detr.model.build_model`` but produces ``SimDETRWithTiedDualDQCGP``
    ...
    if args.a_feat_dir is None:
        model = SimDETRWithTiedDualDQCGP(**common_kwargs)
    else:
        model = SimDETRWithTiedDualDQCGP(
