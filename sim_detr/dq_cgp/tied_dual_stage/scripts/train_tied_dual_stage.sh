...
t_feat_type=clip
results_root=results_dq_cgp/tied_dual_stage_flexible
exp_id=sim_detr_tied_dual_dq_cgp
...
# ---------------------------------------------------------------------------
# Parameter-tied dual-stage DQ-CGP hyperparameters
# ---------------------------------------------------------------------------
...
query_cgp_late_after_layer=2 # second call: D3 → the same DQ → D4
...
# Run
# ---------------------------------------------------------------------------
"${python_bin}" -m sim_detr.dq_cgp.tied_dual_stage.train_dq_cgp \
  --seed ${seed} \
