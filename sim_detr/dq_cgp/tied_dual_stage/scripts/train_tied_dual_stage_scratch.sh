#!/usr/bin/env bash
# Controlled tied stacking experiment:
#   single-stage V3 vs. the same DQ-CGP reused after D1 and D3
#
# This script deliberately has no --init_from/--resume path.  It matches the
# single-stage scratch protocol and changes only the second adapter call.

set -euo pipefail

if (( $# != 0 )); then
  echo "This canonical scratch script accepts no CLI arguments." >&2
  echo "Use TIED_DQCGP_* variables documented in tied_dual_stage/README.md." >&2
  exit 2
fi
...
seed="${TIED_DQCGP_SEED:-2017}"
gpu_id="${TIED_DQCGP_GPU_ID:-0}"
num_workers="${TIED_DQCGP_NUM_WORKERS:-4}"
results_root="${TIED_DQCGP_RESULTS_ROOT:-${repo_root}/results_dq_cgp/tied_dual_stage_scratch/runs}"
exp_id="${TIED_DQCGP_EXP_ID:-sim_detr_tied_dual_dq_cgp_d1_d3_scratch_seed${seed}}"
...
[[ "${exp_id}" =~ ^[A-Za-z0-9._-]+$ ]] || {
  echo "TIED_DQCGP_EXP_ID may contain only letters, numbers, '.', '_' and '-'." >&2
  exit 2
}
...
echo "Starting Sim-DETR + tied dual-stage DQ-CGP from scratch"
...
# method change is reusing the early adapter after D3; early/late auxiliary
# losses are averaged so the global 0.2/0.01 loss budget stays unchanged.
exec "${python_bin}" -m sim_detr.dq_cgp.tied_dual_stage.train_dq_cgp \
  --dset_name hl \
  --ctx_mode video_tef \
