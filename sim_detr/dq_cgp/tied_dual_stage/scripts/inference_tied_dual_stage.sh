#!/usr/bin/env bash
# Strict inference for a Sim-DETR + parameter-tied dual-stage DQ-CGP checkpoint.
#
# Usage:
#   bash sim_detr/dq_cgp/tied_dual_stage/scripts/inference_tied_dual_stage.sh \
#       CHECKPOINT SPLIT [JSONL] [extra args]
# Example:
#   bash sim_detr/dq_cgp/tied_dual_stage/scripts/inference_tied_dual_stage.sh \
...
"${python_bin}" -m sim_detr.dq_cgp.tied_dual_stage.inference_dq_cgp \
  --resume "${checkpoint}" \
