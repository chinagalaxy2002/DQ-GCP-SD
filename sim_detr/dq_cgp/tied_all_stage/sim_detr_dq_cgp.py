"""Sim-DETR with one DQ-CGP reused between every decoder layer.

This class **subclasses SimDETR** without touching any original source file.
For the official four-layer decoder it injects the shared module after D1,
D2 and D3 by:
...
3. Overriding ``forward`` to:
   ...
   - Snapshot separate early/middle/late diagnostics for averaged V3 losses.
...
class SimDETRWithTiedAllStageDQCGP(SimDETR):
    """Sim-DETR extended with a parameter-tied three-call DQ-CGP adapter.
    ...
    query_cgp_middle_after_layer : int
        Middle DQ-CGP insertion layer (0-indexed).  Default 1.
    ...
    def __init__(
        ...
        query_cgp_middle_after_layer: int = 1,
        ...
        self.query_cgp_middle_after_layer = int(query_cgp_middle_after_layer)
        ...
        if use_query_cgp:
            ...
            insertion_layers = (
                self.query_cgp_after_layer,
                self.query_cgp_middle_after_layer,
                self.query_cgp_late_after_layer,
            )
            if len(set(insertion_layers)) != len(insertion_layers):
                raise ValueError(
                    "early, middle and late DQ-CGP insertion layers must differ"
                )

            # The sole registered DQ-CGP parameter set is reused at all gaps.
            ...
            install_dq_cgp_decoder_hooks(
                ...
                adapters_by_layer={
                    ...
                    self.query_cgp_middle_after_layer: self.query_cgp,
    ...
    def forward(
        ...
        if self.use_query_cgp and self.query_cgp is not None:
            ...
            middle_output = stage_outputs.get(self.query_cgp_middle_after_layer)
            ...
            if middle_output is not None:
                out["query_cgp_middle_temporal_attention"] = (
                    middle_output.temporal_attention
                )
                out["query_cgp_middle_basis_weights"] = middle_output.basis_weights
            ...
            if any(
                output is not None
                for output in (early_output, middle_output, late_output)
            ):
...
__all__ = ["ARCHITECTURE_ID", "SimDETRWithTiedAllStageDQCGP"]
