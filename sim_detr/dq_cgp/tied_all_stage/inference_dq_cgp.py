"""Strict checkpoint inference for Sim-DETR + tied all-interlayer DQ-CGP.
...
from .decoder_wrapper import set_dq_cgp_disabled_layers
...
def _requested_eval_overrides(argv):
    ...
    parser.add_argument("--eval_disable_query_cgp_early", action="store_true")
    parser.add_argument("--eval_disable_query_cgp_middle", action="store_true")
    parser.add_argument("--eval_disable_query_cgp_late", action="store_true")
...
def parse_inference_options(argv=None):
    ...
    opt.eval_disable_query_cgp_early = overrides.eval_disable_query_cgp_early
    opt.eval_disable_query_cgp_middle = overrides.eval_disable_query_cgp_middle
    opt.eval_disable_query_cgp_late = overrides.eval_disable_query_cgp_late
...
def setup_model_dq_cgp_inference(opt):
    ...
    validate_tied_checkpoint(checkpoint, opt, operation="Inference")
    model.load_state_dict(checkpoint["model"], strict=True)
    disabled_layers = []
    if opt.eval_disable_query_cgp_early:
        disabled_layers.append(model.query_cgp_after_layer)
    if opt.eval_disable_query_cgp_middle:
        disabled_layers.append(model.query_cgp_middle_after_layer)
    if opt.eval_disable_query_cgp_late:
        disabled_layers.append(model.query_cgp_late_after_layer)
    set_dq_cgp_disabled_layers(model.transformer.decoder, disabled_layers)
    model.to(opt.device)
