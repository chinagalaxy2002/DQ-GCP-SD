#!/usr/bin/env bash
# Controlled all-interlayer tied experiment:
#   the same DQ-CGP is reused after D1, D2 and D3
#
# This script deliberately has no --init_from/--resume path.  It matches the
# single-stage scratch protocol and changes only the number of adapter calls.

set -euo pipefail

if (( $# != 0 )); then
  echo "This canonical scratch script accepts no CLI arguments." >&2
  echo "Use TIED_ALL_DQCGP_* variables documented in tied_all_stage/README.md." >&2
  exit 2
fi
...
seed="${TIED_ALL_DQCGP_SEED:-2017}"
gpu_id="${TIED_ALL_DQCGP_GPU_ID:-0}"
num_workers="${TIED_ALL_DQCGP_NUM_WORKERS:-4}"
results_root="${TIED_ALL_DQCGP_RESULTS_ROOT:-${repo_root}/results_dq_cgp/tied_all_stage_scratch/runs}"
exp_id="${TIED_ALL_DQCGP_EXP_ID:-sim_detr_tied_all_stage_dq_cgp_d1_d2_d3_scratch_seed${seed}}"
...
[[ "${exp_id}" =~ ^[A-Za-z0-9._-]+$ ]] || {
  echo "TIED_ALL_DQCGP_EXP_ID may contain only letters, numbers, '.', '_' and '-'." >&2
  exit 2
}
...
echo "Starting Sim-DETR + tied all-interlayer DQ-CGP from scratch"
echo "seed=${seed} gpu_id=${gpu_id} exp_id=${exp_id}"
echo "results_root=${results_root}"

# Every Sim-DETR argument below matches the single-stage scratch run.  The only
# method change is reusing one adapter after D1, D2 and D3. The three stage
# losses are averaged so the global 0.2/0.01 loss budget stays unchanged.
exec "${python_bin}" -m sim_detr.dq_cgp.tied_all_stage.train_dq_cgp \
  --dset_name hl \
  --ctx_mode video_tef \
