...
Run from the Sim-DETR project root:
    PYTHONPATH=. python -m pytest sim_detr/dq_cgp/tied_all_stage/tests/test_loss.py -v
"""

import torch
from torch import nn

from sim_detr.dq_cgp.tied_all_stage.loss import (
    compute_binding_loss,
    compute_routing_loss,
    loss_query_cgp,
)
from sim_detr.dq_cgp.tied_all_stage.model_builder import DQCGPSetCriterion
...
class TestCombinedLoss:
    def _make_outputs(self, attn_val=None):
        ...
        return {
            ...
            "query_cgp_middle_temporal_attention": attn.clone(),
            "query_cgp_middle_basis_weights": torch.softmax(
                torch.randn(BSZ, NQ, NK), dim=-1
            ),
            "query_cgp_late_temporal_attention": attn.clone(),
            "query_cgp_late_basis_weights": torch.softmax(
                torch.randn(BSZ, NQ, NK), dim=-1
            ),
    ...
    def test_route_loss_ignores_unmatched_queries(self):
        ...
        for outputs in (outputs_a, outputs_b):
            for key in (
                ...
                outputs[key].zero_()
                outputs[key][0, 0, 0] = 1.0
                outputs[key][1, 0, 1] = 1.0
        ...
        outputs_a["query_cgp_middle_basis_weights"][:, 1:, 4] = 1.0
        outputs_b["query_cgp_middle_basis_weights"][:, 1:, 5] = 1.0
        outputs_a["query_cgp_late_basis_weights"][:, 1:, 3] = 1.0
        outputs_b["query_cgp_late_basis_weights"][:, 1:, 6] = 1.0
        indices = [
    ...
    def test_routing_is_computed_within_each_stage_before_averaging(self):
        outputs = self._make_outputs()
        # Each stage is individually collapsed, but onto a different basis.
        # A cross-stage concatenation would incorrectly report diversity.
        outputs["query_cgp_basis_weights"].zero_()
        outputs["query_cgp_basis_weights"][..., 0] = 1.0
        outputs["query_cgp_middle_basis_weights"].zero_()
        outputs["query_cgp_middle_basis_weights"][..., 2] = 1.0
        outputs["query_cgp_late_basis_weights"].zero_()
        outputs["query_cgp_late_basis_weights"][..., 1] = 1.0
        indices = _uniform_indices(1)
        result = loss_query_cgp(
            outputs, _make_gt_targets([(0.5, 0.3)]), indices
        )
        assert abs(result["loss_query_cgp_route"].item()) < 1e-7

    def test_stage_losses_are_separate_then_exactly_averaged(self):
        outputs = self._make_outputs()

        # Use integer CE spans so the target interval is exact.  Early puts all
        # matched mass inside [10, 19], while late puts exactly one quarter in
        # it, giving stage binding losses 0 and -log(0.25), respectively.
        early_attention = torch.zeros(BSZ, NQ, TV)
        early_attention[..., 0] = 1.0
        early_attention[:, 0, 0] = 0.0
        early_attention[:, 0, 10:20] = 0.1
        late_attention = early_attention.clone()
        late_attention[:, 0, 10:20] = 0.025
        late_attention[:, 0, 0] = 0.75
        outputs["query_cgp_temporal_attention"] = early_attention
        outputs["query_cgp_middle_temporal_attention"] = early_attention.clone()
        outputs["query_cgp_late_temporal_attention"] = late_attention

        # Early is collapsed.  Late sends the two matched samples to different
        # bases, giving routing losses 0 and -log(2).
        early_routes = torch.zeros(BSZ, NQ, NK)
        early_routes[..., 0] = 1.0
        late_routes = early_routes.clone()
        late_routes[1, 0].zero_()
        late_routes[1, 0, 1] = 1.0
        outputs["query_cgp_basis_weights"] = early_routes
        outputs["query_cgp_middle_basis_weights"] = early_routes.clone()
        outputs["query_cgp_late_basis_weights"] = late_routes

        targets = {
            "span_labels": [
                {"spans": torch.tensor([[10, 19]], dtype=torch.long)}
                for _ in range(BSZ)
            ]
        }
        result = loss_query_cgp(
        ...
        late_bind = -torch.log(torch.tensor(0.25))
        late_route = -torch.log(torch.tensor(2.0))
        assert torch.allclose(result["query_cgp_early_bind_raw"], zero, atol=1e-6)
        assert torch.allclose(result["query_cgp_middle_bind_raw"], zero, atol=1e-6)
        assert torch.allclose(
            result["query_cgp_late_bind_raw"], late_bind, atol=1e-6
        )
        assert torch.allclose(result["query_cgp_early_route_raw"], zero, atol=1e-6)
        assert torch.allclose(result["query_cgp_middle_route_raw"], zero, atol=1e-6)
        assert torch.allclose(
            result["query_cgp_late_route_raw"], late_route, atol=1e-6
        )
        assert torch.allclose(
            result["loss_query_cgp_bind"], 0.2 * late_bind / 3.0, atol=1e-6
        )
        assert torch.allclose(
            result["loss_query_cgp_route"], 0.01 * late_route / 3.0, atol=1e-6
        )
    ...
    def test_no_matches_returns_graph_connected_diagnostic_zeros(self):
        ...
        outputs["query_cgp_middle_temporal_attention"].requires_grad_()
        outputs["query_cgp_middle_basis_weights"].requires_grad_()
        outputs["query_cgp_late_temporal_attention"].requires_grad_()
        outputs["query_cgp_late_basis_weights"].requires_grad_()
        empty_indices = [
            (torch.empty(0, dtype=torch.long), torch.empty(0, dtype=torch.long))
            for _ in range(BSZ)
        ]
        ...
        assert outputs["query_cgp_middle_temporal_attention"].grad is not None
        assert outputs["query_cgp_middle_basis_weights"].grad is not None
        assert outputs["query_cgp_late_temporal_attention"].grad is not None
        assert outputs["query_cgp_late_basis_weights"].grad is not None
...
class TestCriterionIntegration:
    def test_real_targets_and_aux_skip(self):
        ...
        assert set(losses) == {
            "loss_query_cgp_bind",
            "loss_query_cgp_route",
            "query_cgp_early_bind_raw",
            "query_cgp_middle_bind_raw",
            "query_cgp_late_bind_raw",
            "query_cgp_early_route_raw",
            "query_cgp_middle_route_raw",
            "query_cgp_late_route_raw",
        }
