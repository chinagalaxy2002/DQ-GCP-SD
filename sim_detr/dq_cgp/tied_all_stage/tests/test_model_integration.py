...
from sim_detr.dq_cgp.tied_all_stage.utils import masked_mean_pool
...
def make_model(run_native_decoder=False):
    ...
    def fake_transformer_forward(
        ...
        if run_native_decoder:
            memory_sequence_first = src[:, :video_length].transpose(0, 1)
            position_sequence_first = pos_embed[:, :video_length].transpose(0, 1)
            target = sent_feat.unsqueeze(0).expand(num_queries, -1, -1)
            hs, references = transformer_self.decoder(
                target,
                memory_sequence_first,
                memory_key_padding_mask=mask[:, :video_length],
                pos=position_sequence_first,
                refpoints_unsigmoid=query_embed.unsqueeze(1).expand(
                    -1, batch_size, -1
                ),
            )
            memory = memory_sequence_first.transpose(0, 1)
...
class TestSemanticMask:
    def test_model_exposes_all_three_preserved_stage_snapshots(self):
        model = make_model(run_native_decoder=True)
        src_txt, src_txt_mask, src_vid, src_vid_mask = make_forward_inputs()

        with torch.no_grad():
            outputs = model(src_txt, src_txt_mask, src_vid, src_vid_mask)

        stage_outputs = get_dq_cgp_stage_outputs(model.transformer.decoder)
        early = stage_outputs[model.query_cgp_after_layer]
        middle = stage_outputs[model.query_cgp_middle_after_layer]
        late = stage_outputs[model.query_cgp_late_after_layer]
        assert len({id(early), id(middle), id(late)}) == 3
        assert outputs["query_cgp_temporal_attention"] is early.temporal_attention
        assert outputs["query_cgp_basis_weights"] is early.basis_weights
        assert (
            outputs["query_cgp_middle_temporal_attention"]
            is middle.temporal_attention
        )
        assert outputs["query_cgp_middle_basis_weights"] is middle.basis_weights
        assert (
            outputs["query_cgp_late_temporal_attention"]
            is late.temporal_attention
        )
        assert outputs["query_cgp_late_basis_weights"] is late.basis_weights
        assert torch.equal(outputs["query_cgp_video_mask"], src_vid_mask.bool())
