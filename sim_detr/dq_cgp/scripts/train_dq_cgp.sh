#!/usr/bin/env bash
# ============================================================================
# train_dq_cgp.sh  –  flexible Sim-DETR + DQ-CGP V3 utility
#
# The canonical from-scratch effectiveness experiment is locked down in:
#   scripts/train_dq_cgp_v3_scratch.sh
#
...
#
# Architecture:  D1 → DQ-CGP → D2 → D3 → D4
...
t_feat_type=clip
results_root=results_dq_cgp
exp_id=sim_detr_dq_cgp_v3
...
# Run
# ---------------------------------------------------------------------------
"${python_bin}" -m sim_detr.dq_cgp.train_dq_cgp \
  --seed ${seed} \
